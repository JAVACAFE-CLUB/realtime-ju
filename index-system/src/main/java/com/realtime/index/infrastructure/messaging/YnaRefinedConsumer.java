...
import com.realtime.index.application.processor.YnaIndexProcessor;
...
/**
 * YNA refined 토픽 Consumer
 */
@Component
@RequiredArgsConstructor
@Slf4j
public class YnaRefinedConsumer {

    private final YnaIndexProcessor processor;

    @KafkaListener(
            topics = KafkaTopics.REFINED_NEWS_YNA,
    ...
    public void consume(
        ...
        log.info("📥 Kafka Received: {} YNA messages from partitions [{}], offset range [{}-{}]",
        ...
        try {
            ...
            ack.acknowledge();
            log.info("✅ Committed {} YNA messages successfully", messages.size());

        } catch (Exception e) {
            log.error("❌ Failed to process YNA refined messages", e);
            throw new RuntimeException("YNA consumer failed", e);
        }
    }
}
