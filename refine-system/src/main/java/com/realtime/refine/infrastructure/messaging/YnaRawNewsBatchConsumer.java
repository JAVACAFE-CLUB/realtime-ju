...
import com.realtime.refine.application.news.yna.YnaRefineProcessor;
...
@Slf4j
public class YnaRawNewsBatchConsumer {

    private final YnaRefineProcessor processor;

    // [수신 설정] RAW_NEWS_YNA 토픽에서 배치 리스닝
    @KafkaListener(
            topics = KafkaTopics.RAW_NEWS_YNA,
    ...
    public void onBatch(List<ConsumerRecord<String, CollectMessage>> records, Acknowledgment ack) {
        ...
        // [2단계] 배치 정보 로깅
        log.info("📥 YNA 배치 수신 - size={}, partitions={}", 
                records.size(), 
        ...
        try {
            ...
            log.error("❌ YNA 배치 처리 실패 - size={}, error={}", records.size(), e.getMessage(), e);

        } finally {
            // [마감] 처리 소요시간 및 배치 크기 기록
            long took = System.currentTimeMillis() - start;
            double avgMs = records.isEmpty() ? 0 : (double) took / records.size();
            log.info("⏱️ YNA 배치 처리 완료 - size={}, success={}, fail={}, totalTime={}ms, avgTime={:.2f}ms/msg", 
